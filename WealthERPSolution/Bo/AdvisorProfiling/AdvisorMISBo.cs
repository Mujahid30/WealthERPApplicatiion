...
namespace BoAdvisorProfiling
{
    public class AdvisorMISBo
    {
        ...
        public DataSet GetCommissionReceivableRecon(string product, int typeOfTransaction, int AdviserId, int schemeid, int month, int year, string category, string recontype, string commtype, int issuer, int issueId, int commissionLookUpId)
        {
            ...
            try
            {
                dsGetCommissionReconMis = MISDao.GetCommissionReceivableRecon(product, typeOfTransaction, AdviserId, schemeid, month, year, category, recontype, commtype, issuer, issueId, commissionLookUpId);
