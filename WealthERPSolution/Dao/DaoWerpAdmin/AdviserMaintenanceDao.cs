    ...
    public class AdviserMaintenanceDao
    {
        ...
        public void MessageBroadcastSendMessage(string BroadcastMessage, DateTime Broadcasttime, DateTime ExpiryDate, string gvAdviserIds)
        {
            ...
            try
            {
                ...
                if (ExpiryDate != DateTime.MinValue)
                    dbMessageBroadcast.AddInParameter(CmdMessageBroadcast, "@ExpiryDate", DbType.DateTime, ExpiryDate);
                else
                    dbMessageBroadcast.AddInParameter(CmdMessageBroadcast, "@ExpiryDate", DbType.DateTime, DBNull.Value);
        ...
        public List<AdvisorVo> GetAdviserListWithPager(string SortExpression, string filterExpression,string ifaNameSearch)
        {
            ...
            try
            {
                ...
                db.AddInParameter(Cmd, "@IFAName", DbType.String, DBNull.Value);
        ...
        public List<AdvisorVo> GetIFFListForUserManagement(int CurrentPage, out int Count, string SortExpression, string IffNameFilter)
        {
            ...
            {
                db = DatabaseFactory.CreateDatabase("wealtherp");
                Cmd = db.GetStoredProcCommand("SP_GetIFFsForUserManagement");
                db.AddInParameter(Cmd, "@CurrentPage", DbType.Int32, CurrentPage);
                db.AddInParameter(Cmd, "@SortOrder", DbType.String, SortExpression);
                db.AddInParameter(Cmd, "@nameFilter", DbType.String, IffNameFilter);
                getAdvisorDs = db.ExecuteDataSet(Cmd);
                dtAdvisers = getAdvisorDs.Tables[0];
                foreach (DataRow dr in dtAdvisers.Rows)
                {
