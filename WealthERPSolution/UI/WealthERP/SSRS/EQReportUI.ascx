<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="EQReportUI.ascx.cs"
    Inherits="WealthERP.SSRS.EQReportUI" %>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#ctrl_EQReportUI_btnView').bubbletip($('#div1'), { deltaDirection: 'left' });
        $('#ctrl_EQReportUI_btnViewInPDF').bubbletip($('#div2'), { deltaDirection: 'left' });
        $('#ctrl_EQReportUI_btnViewInExcel').bubbletip($('#div3'), { deltaDirection: 'left' });
    });
</script>
...
<script type="text/javascript" language="javascript">
    function validate(type) {
        ...
        if (type == 'pdf')
            window.document.forms[0].action = "/SSRS/DisplayEQReport.aspx?format=pdf";
        else if (type == 'xls')
            window.document.forms[0].action = "/SSRS/DisplayEQReport.aspx?format=xls";
        else
            window.document.forms[0].action = "/SSRS/DisplayEQReport.aspx?format=view";
    ...
    function ChangeDates() {
        ...
        if (selectedReport != "EQ_TRANSACTION" && selectedReport != "CAPITAL_GAIN_DETAILS" && selectedReport != "CAPITAL_GAIN_SUMMARY" ) {
...
<div class="divPageHeading" style="height: 30px;">
    <table border="0" width="100%">
        <tr>
            <td>
                <asp:Label ID="Label7" runat="server" Text="EQ Report"></asp:Label>
                <hr />
            </td>
        </tr>
    </table>
    ...
    <tr id="trAdminRmButton" runat="server">
        <td colspan="2" align="right">
            <asp:Button ID="btnViewInPDF" runat="server" OnClientClick="return validate('pdf')"
                PostBackUrl="~/SSRS/DisplayEQReport.aspx?format=pdf" CssClass="PDFButton" />&nbsp;&nbsp;
            <div id="div2" style="display: none;">
                <p class="tip">
                    Click here to download report in pdf format.
                </p>
            </div>
            <asp:Button ID="btnView" runat="server" OnClientClick="return validate('view')" PostBackUrl="~/SSRS/DisplayEQReport.aspx?format=view"
            ...
            <asp:Button ID="btnViewInExcel" runat="server" CssClass="ExcelButton" OnClientClick="return validate('xls')"
                PostBackUrl="~/SSRS/DisplayEQReport.aspx?format=xls" />&nbsp;&nbsp;
                ...
                <tr>
                    ...
                    <td>
                        <asp:DropDownList ID="ddlReportSubType" runat="server" CssClass="cmbField" onChange="ChangeDates()"
                            ...
                            <asp:ListItem Text="" Value="EQ_HOLDING_REPORT" Selected="True">Equity Holding Report</asp:ListItem>
                            ...
                            <asp:ListItem Text="" Value="EQ_TRANSACTION">Transaction Report</asp:ListItem>
                        </asp:DropDownList>
                    </td>
                </tr>
            </table>
