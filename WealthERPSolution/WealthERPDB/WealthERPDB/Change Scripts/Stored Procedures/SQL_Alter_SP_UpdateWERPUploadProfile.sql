...
ALTER PROCEDURE [dbo].[SP_UpdateWERPUploadProfile]
...
AS
	...
	If (@@Trancount = 0)  
	 ...
	 BEGIN
	 	UPDATE dbo.CustomerMFXtrnlProfileStaging 
		SET 
			CMFXPS_FolioNum = @Folio,
			CMFXPS_PANNum = @PanNumber
		WHERE 
			CMFXPS_Id = @CMFXPS_Id
	 END
	 ...
	 BEGIN
	 	...
	 	UPDATE CustomerMFXtrnlProfileStaging 
		SET 
			CMFXPS_PANNum = @PanNumber
		WHERE 
			CMFXPS_Id = @CMFXPS_Id
